...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Decode farm() transaction sub method calls.')
    ...
    for arg_name, arg_value in decoded_txn[1].items():
        ...
        if decoded_txn[0].function_identifier == 'farm' and arg_name == 'data':
            for sub_method_call_bytes in arg_value:
                ...
                decoded_sub_method_call = contract.decode_function_input(
                    sub_method_call_bytes)
                logging.info(
                    f'decoded_sub_method_call: {decoded_sub_method_call}')
        else:

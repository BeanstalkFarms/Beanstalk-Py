...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Search for event in recent transactions.')
    ...
    parser.add_argument('-a', '--address', type=str,
                        default='0xC1E088fC1323b20BCBee9bd1B9fC9546db5624C5', help='the contract to check events at (default: Beanstalk addr)')
    parser.add_argument('-b', '--max_block', type=int, default=12974075,
    ...
    while len(entry_matches) < args.num_txns and latest_searched_block > args.max_block:
        ...
        logging.info(
            f'Checking blocks {from_block} to {to_block} for {args.event_definition}')
    ...
    if len(entry_matches) == 0:
        print(
            f'No transactions found with matching event definition - {args.event_definition}')
    else:

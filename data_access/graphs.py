...
class BeanstalkSqlClient(object):
    ...
    def silo_assets_seasonal_changes(self, current_silo_assets=None, previous_silo_assets=None):
        ...
        if len(current_silo_assets) != len(previous_silo_assets):
            logging.warning(
                'Number of assets in this season changed. Was a new asset added?')
            return []

        assets_changes = []
        for i in range(len(previous_silo_assets)):
            assets_changes.append(AssetChanges(
                previous_silo_assets[i], current_silo_assets[i]))
...
class SeasonStats():
    ...
    def __init__(self, graph_seasons_response, season_index=0, season=None):
        ...
        if 'seasons' in graph_seasons_response:
            ...
            self.price = float(
                graph_seasons_response['seasons'][season_index]['price'])
            ...
            self.delta_b = bean_to_float(
                graph_seasons_response['seasons'][season_index]['deltaB'])
            self.total_beans = bean_to_float(
                graph_seasons_response['seasons'][season_index]['beans'])  # Bean supply
...
class AssetChanges():
    ...
    def __init__(self, init_season_asset, final_season_asset):
        ...
        self.delta_asset = int(
            final_season_asset['depositedAmount']) - int(init_season_asset['depositedAmount'])
        # self.delta_asset_percent = (
        #     int(final_season_asset['depositedAmount']) /
        #     int(init_season_asset['depositedAmount']) - 1) * 100
        self.delta_bdv = int(
            final_season_asset['depositedBDV']) - int(init_season_asset['depositedBDV'])
